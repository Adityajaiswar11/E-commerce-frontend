...
const Slide = () => {
  ...
  //automatically slider change function
  useEffect(() => {
  ...
  return (
    <>
    <marquee  direction="down" className="text-2xl mt-5 py-4 bg-blue-600/70 text-center">SALE ! SALE ! 50% OFF</marquee>
    <div className="slide w-[90%] bg-[#4158D0] mx-auto mt-20 text-center flex justify-center items-center md:h-[70vh] bg-gradient-to-tl h-[50vh] rounded-full">
     <h2 className="text-3xl font-semibold md:block hidden">TOP DEALS</h2>
      {data.map((img, i) => {
        return (
          <div className={`slider ${count == i ? "block" : "hidden"} p-3`} key={i}>
            <img src={img.pic} alt="img"  width={800} className=" rounded-md"/>
            <div className="btn">
              <div className="icons bg-red-600 hover:bg-blue-600 duration-200" onClick={handleClickminus}>
                <FaArrowLeftLong />
              </div>

              <div className="icons bg-red-600 hover:bg-pink-600 duration-200" onClick={handleClickPlus}>
                <FaArrowRightLong />
              </div>
            </div>
          </div>
